...
class `Command` and `AddGroupCommandParser`, which implements the `Parser` interface. The command creates a new `Group`
instance, which is then stored in an instance of `UniqueGroupList`, which in turn is stored in the `AddressBook`.
