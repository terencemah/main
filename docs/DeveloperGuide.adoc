...
Given below is an activity diagram shows how the import operation works with the command `import l/life.csv g/group.csv e/event.csv`:
...
`ImportFile#importEventCsv`to check if the CSV file headers are valid.
 qq4. `ImportCommand` would then call `Model#hasPersons`, `Model#hasGroups` and `Model#hasEvents` to check if the persons, groups and events are duplicates of the current CoderLifeInsights.
4. If the imported data are not duplicates, it will then create a valid list of persons, groups and events to be added to CoderLifeInsights.
5. CoderLifeInsights will then populate the 3 lists to the current data.
...
Given below is an activity diagram illustrating how the export operation works with the command `export l/life.csv g/group.csv e/event.csv`
...
Given below is an activity diagram illustrating how the suggestion operation works with the command `suggest person`.
...
2. `SuggestCommand` would then check if keyword entered corresponds to `person`, `place` or `activity`.
3. Upon valid keyword entered, `SuggestCommand` would call `Model#suggestPerson` to return a suggested person.
4. ObservableList of filteredPersons would be iterated to get suggested person based on least time spent and reverse lexicographical order of name as a tie breaker.
...
`Model#suggestPlace` or `Model#suggestActivity` where an ObservableList of filteredEvents would be iterated to get suggested place or activity based on the least frequency of the place visited or activity done.

If there are multiple place visited or activity done has the same frequency, a tie breaker would be introduced where the first place/activity would be suggested based on chronological order.
//end::suggest[]
...
To allow users to track social activities with a group of people, the `add_group` feature is implemented.
The feature uses the `AddressBook` class stored in `ModelManager` instance of the app.
